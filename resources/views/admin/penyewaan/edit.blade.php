...
<style>
    ...
    /* File existing */
    .file-existing { display:flex; align-items:center; gap:8px; padding:8px 12px; background:var(--bg-hover); border:1px solid var(--border); border-radius:8px; margin-bottom:8px; }
    .file-existing i { font-size:16px; color:var(--brand-500); flex-shrink:0; }
    .file-existing-name { font-size:12.5px; color:var(--text-primary); font-weight:500; flex:1; overflow:hidden; text-overflow:ellipsis; white-space:nowrap; }
    .file-existing a { font-size:12px; color:var(--brand-500); text-decoration:none; white-space:nowrap; display:inline-flex; align-items:center; gap:3px; }
    .file-existing a:hover { text-decoration:underline; }
    .file-existing-hint { font-size:11.5px; color:var(--text-muted); margin-top:4px; display:flex; align-items:center; gap:4px; }
...
<div style="display:flex; align-items:center; gap:12px; margin-bottom:24px;">
    ...
    <div>
        ...
        <p style="font-size:13px; color:var(--text-muted);">Perbarui data penyewaan atas nama <strong>{{ $penyewaan->nama_penyewa }}</strong></p>
    </div>
</div>
            ...
            <div class="form-grid">
                ...
                <div class="form-group" style="grid-column:1/-1;">
                    ...
                    @php
                        ...
                        // old() saat validasi gagal, fallback ke data DB (split CSV)
                        $savedAlkes = old('produk_alkes',
                    ...
                    <div class="checkbox-grid">
                        ...
                        <label class="checkbox-item">
                            <input type="checkbox" name="produk_alkes[]" value="{{ $alkes }}"
                                   {{ in_array($alkes, $savedAlkes) ? 'checked' : '' }}>
                ...
                <div class="form-group" style="grid-column:1/-1;">
                    ...
                    <div class="durasi-display" id="durasi-display">
                        @php $durasiAwal = old('durasi_hari', $penyewaan->durasi_hari); @endphp
                        @if($durasiAwal)
                            <i class="ri-calendar-check-line"></i> {{ $durasiAwal }} hari
                        @endif
                    </div>
                    @error('tgl_mulai')
                        <span class="invalid-feedback"><i class="ri-error-warning-line"></i> {{ $message }}</span>
                    ...
                    <div class="form-group">
                        ...
                        <select name="pengiriman" id="pengiriman"
                            ...
                            <option value="mandiri"
                                {{ old('pengiriman', $penyewaan->pengiriman) == 'mandiri' ? 'selected' : '' }}>
                                Ambil dan Antar kembali sendiri oleh Penyewa
                            </option>
                            <option value="Gosend / GrabExpress"
                                {{ old('pengiriman', $penyewaan->pengiriman) == 'Gosend / GrabExpress' ? 'selected' : '' }}>
                                via Gosend / GrabExpress (barang dibawah 2 Kg, ex. Nebulizer / Kursi Roda Travel / sejenisnya)
                            </option>
                            <option value="Rental Mobil Paralkes"
                                {{ old('pengiriman', $penyewaan->pengiriman) == 'Rental Mobil Paralkes' ? 'selected' : '' }}>
        ...
        <div class="form-section">
            ...
            <div class="form-grid form-grid-2">
                ...
                <div class="form-group">
                    ...
                    <select name="metode_pembayaran" id="metode_pembayaran"
                        ...
                        <option value="Tunai / Cash"
                            {{ old('metode_pembayaran', $penyewaan->metode_pembayaran) == 'Tunai / Cash' ? 'selected' : '' }}>
                            Tunai / Cash
                        </option>
                        <option value="Transfer via Bank BCA"
                            {{ old('metode_pembayaran', $penyewaan->metode_pembayaran) == 'Transfer via Bank BCA' ? 'selected' : '' }}>
                ...
                <div class="form-group">
                    ...
                    <select name="status"
                        ...
                        <option value="berjalan"
                            {{ old('status', $penyewaan->status) == 'berjalan' ? 'selected' : '' }}>
                            Berjalan
                        </option>
                        <option value="segera_konfirmasi"
                            {{ old('status', $penyewaan->status) == 'segera_konfirmasi' ? 'selected' : '' }}>
                            Segera Konfirmasi
                        </option>
                        <option value="selesai"
                            {{ old('status', $penyewaan->status) == 'selesai' ? 'selected' : '' }}>
                            Selesai
                        </option>
                    </select>
                    @error('status')
                ...
                <div class="form-group">
                    ...
                    {{-- Tampilkan file existing jika sudah ada --}}
                    @if($penyewaan->foto_ktp_sim)
                    <div class="file-existing" id="file-existing-ktp">
                        <i class="ri-id-card-line"></i>
                        <span class="file-existing-name">File sudah tersimpan</span>
                        <a href="{{ asset('storage/' . $penyewaan->foto_ktp_sim) }}" target="_blank">
                            <i class="ri-eye-line"></i> Lihat
                        </a>
                    </div>
                    <div class="file-existing-hint">
                        <i class="ri-information-line"></i>
                        Upload file baru di bawah ini untuk mengganti file lama
                    </div>
                    @endif
                    ...
                    <div class="dropzone" id="dropzone-ktp"
                         ...
                         style="{{ $penyewaan->foto_ktp_sim ? 'margin-top:8px;' : '' }}">
                        <input type="file"
                               id="foto_ktp_sim"
                               name="foto_ktp_sim"
                        ...
                        <div class="dropzone-title">
                            {{ $penyewaan->foto_ktp_sim ? 'Klik atau seret untuk mengganti file' : 'Klik atau seret file ke sini' }}
                        </div>

            ...
            <table class="items-table" id="items-table">
                <thead>
                    <tr>
                        ...
                        <th style="width:70px;">Qty <span style="color:#fde68a">*</span></th>
...
/* ── Template hasil dropdown Select2 ── */
...
function addRow(data = {}) {
    ...
    tr.innerHTML = `
        ...
        <td style="min-width:260px">
            {{-- hidden fields wajib untuk backend --}}
            ...
            {{-- Select2 pilih barang dari inventory --}}
            <select id="inv-select-${idx}" class="form-control" required>
                ${data.inventory_id
                    ? `<option value="${data.inventory_id}" selected>${data.nama_barang || ''}</option>`
                    : ''}
            </select>

            {{-- Info stok setelah pilih --}}
            <div class="stok-info-row" id="stok-info-${idx}"></div>
        </td>

        <td>
            <select name="items[${idx}][kondisi]" id="kondisi-${idx}"
                    class="form-control" style="width:100px" onchange="onKondisiChange(${idx})">
                ${kondisiOpts}
            </select>
        </td>
        <td>
            <input type="number" name="items[${idx}][qty]"
                   ...
                   class="form-control" style="text-align:center;width:64px" required>
        </td>
        <td>
    ...
    /* ── Init Select2 pada kolom barang ── */
    ...
    .on('select2:open', function () {
        /* auto-trigger supaya langsung muncul semua item tanpa perlu ngetik */
    ...
    .on('select2:select', function (e) {
        ...
        /* update stok badge info di bawah dropdown */
        $(`#stok-info-${idx}`).html(`
            <span class="stok-baru-badge">Baru: ${item.stok_baru ?? 0}</span>
            <span class="stok-bekas-badge">Bekas: ${item.stok_bekas ?? 0}</span>
        ...
        /* set satuan otomatis */
        const $sat = $(`#row-${idx} select[name="items[${idx}][satuan]"]`);
        if (item.satuan) {
        ...
        /* update harga hidden sesuai kondisi saat ini */
    ...
    /* jika ada data existing (edit) — tampilkan stok info langsung */
...
/* ── Update harga hidden berdasarkan kondisi terpilih ── */
...
/* ── Kondisi berubah → update harga hidden dari data Select2 ── */
function onKondisiChange(idx) {
    const sel  = $(`#inv-select-${idx}`);
    const data = sel.select2('data')?.[0];
    if (data && data.id) updateHargaHidden(idx, data);
}

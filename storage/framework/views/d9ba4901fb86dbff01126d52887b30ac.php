...
<style>
    ...
    .action-dropdown-item.item-buyback i { color:#D97706; }
    ...
    html.dark .alert-info { background:rgba(29,78,216,0.1); color:#60A5FA; border-color:rgba(29,78,216,0.25); }
    ...
    .buyback-info-value { font-weight:600; color:var(--text-primary); }
    .form-group { display:flex; flex-direction:column; gap:5px; margin-bottom:14px; }
    .form-label { font-size:12.5px; font-weight:600; color:var(--text-secondary); }
    .form-label .required { color:#EF4444; margin-left:2px; }
    .form-control { height:40px; padding:0 12px; border:1px solid var(--border); border-radius:8px; background:var(--bg-primary); color:var(--text-primary); font-size:13px; font-family:var(--font); outline:none; transition:border-color 0.2s; width:100%; box-sizing:border-box; }
    ...
    .bb-table td { padding:10px 11px; border-bottom:1px solid var(--border); vertical-align:middle; }
...
<div class="table-card">
    ...
    <div class="table-toolbar">
        <div class="toolbar-left">
            ...
            <form method="GET" action="<?php echo e(route('penjualan.index')); ?>" id="perPageForm">
            ...
            <form method="GET" action="<?php echo e(route('penjualan.index')); ?>" class="search-form" id="searchForm">
                ...
                <?php if($search): ?>
                <a href="<?php echo e(route('penjualan.index', ['per_page' => $perPage, 'date_from' => $dateFrom, 'date_to' => $dateTo])); ?>"
                   class="btn btn-reset">
    ...
    <div class="date-filter-bar">
        ...
        <form method="GET" action="<?php echo e(route('penjualan.index')); ?>" id="dateFilterForm"
            ...
            <a href="<?php echo e(route('penjualan.index', ['search' => $search, 'per_page' => $perPage])); ?>"
    ...
    <div class="export-panel" id="exportPanel">
        ...
        <form method="GET" action="<?php echo e(route('penjualan.export')); ?>" id="formExport">
            <input type="hidden" name="search" value="<?php echo e($search); ?>">
            <div class="export-filter-row">
                ...
                <div class="export-filter-group">
                    ...
                    <input type="date" name="date_from" class="export-filter-input"
                           value="<?php echo e($dateFrom ?: request('date_from')); ?>"
                           style="width:150px;">
                </div>
                ...
                <div class="export-filter-group">
                    ...
                    <input type="date" name="date_to" class="export-filter-input"
                           value="<?php echo e($dateTo ?: request('date_to')); ?>"
                           style="width:150px;">
                </div>
                ...
                <div class="export-filter-group">
                    ...
                    <select name="status_pembayaran" class="export-filter-input" style="width:155px;">
                        ...
                        <option value="dp"          <?php echo e(request('status_pembayaran') === 'dp'          ? 'selected' : ''); ?>>DP / Sebagian</option>
                ...
                <div class="export-filter-group">
                    ...
                    <select name="status_transaksi" class="export-filter-input" style="width:140px;">
                        ...
                        <option value="aktif"   <?php echo e(request('status_transaksi') === 'aktif'   ? 'selected' : ''); ?>>Aktif</option>
                        <option value="selesai" <?php echo e(request('status_transaksi') === 'selesai' ? 'selected' : ''); ?>>Selesai</option>
                        <option value="batal"   <?php echo e(request('status_transaksi') === 'batal'   ? 'selected' : ''); ?>>Dibatalkan</option>
                    </select>
                </div>
        ...
        <div class="info-bar-text">
            <?php if($search || $dateFrom || $dateTo): ?>
                ...
                <?php if($dateFrom || $dateTo): ?>
                    ...
                    <?php endif; ?>
                <?php endif; ?>
                &nbsp;<span class="badge-count"><?php echo e($penjualans->total()); ?> transaksi</span>
            <?php else: ?>
                <i class="ri-exchange-dollar-line"></i>
                Total <span class="badge-count"><?php echo e($penjualans->total()); ?> transaksi</span>
            <?php endif; ?>
        </div>
        <?php if($penjualans->total() > 0): ?>
        <div class="info-bar-text">
            Halaman <strong><?php echo e($penjualans->currentPage()); ?></strong>
            dari <strong><?php echo e($penjualans->lastPage()); ?></strong>
        </div>
        <?php endif; ?>
    </div>
            ...
            <tbody>
                ...
                <tr>
                    <td style="color:var(--text-muted);font-size:12.5px;font-weight:500;">
                        <?php echo e($penjualans->firstItem() + $loop->index); ?>
                    ...
                    <td>
                        <span class="tanggal-badge">
                            ...
                            <?php echo e(\Carbon\Carbon::parse($item->tanggal_penjualan)->format('d M Y')); ?>

                        </span>
                    </td>

                    <td>
                        <div style="font-weight:600;font-size:13px;"><?php echo e($item->nama_pelanggan); ?></div>
                        <?php if($item->nomor_telepon): ?>
                        <div style="font-size:11.5px;color:var(--text-muted);margin-top:2px;">
                            <i class="ri-phone-line" style="font-size:11px;"></i> <?php echo e($item->nomor_telepon); ?>

                        </div>
                        <?php endif; ?>
                        <?php if($item->alamat_pelanggan): ?>
                        <div style="font-size:11.5px;color:var(--text-muted);margin-top:1px;max-width:160px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">
                            <i class="ri-map-pin-line" style="font-size:11px;"></i> <?php echo e($item->alamat_pelanggan); ?>
                    ...
                    <td style="min-width:220px;">
                        <?php if($item->details->count() > 0): ?>
                        <div class="items-list">
                            <?php $__currentLoopData = $item->details->take(2); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $d): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <div class="item-pill">
                                <span class="item-name"><?php echo e($d->nama_barang); ?></span>
                                <span class="item-qty">×<?php echo e($d->qty); ?></span>
                                <span class="item-sub">Rp <?php echo e(number_format($d->subtotal, 0, ',', '.')); ?></span>
                            </div>
                            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                            <?php if($item->details->count() > 2): ?>
                            <span class="items-more"
                                  onclick="window.location='<?php echo e(route('penjualan.show', $item->id)); ?>'">
                                <i class="ri-add-circle-line"></i>
                                +<?php echo e($item->details->count() - 2); ?> barang lainnya
                            </span>
                            <?php endif; ?>
                        </div>
                        <?php else: ?>
                    ...
                    <td>
                        ...
                        ?>
                        <span class="pay-badge <?php echo e($payClass); ?>">
                            <i class="<?php echo e($payIcon); ?>"></i>
                            <?php echo e(ucfirst($metode)); ?>

                        </span>
                        <br>
                        <span class="status-badge <?php echo e($statusClass); ?>">
                            <i class="<?php echo e($statusIcon); ?>" style="font-size:10px;"></i>
                    ...
                    <td style="min-width:170px;">
                        <div class="kirim-wrap">
                            <span class="kirim-badge <?php echo e($kirimClass); ?>">
                                <i class="<?php echo e($item->jasa_pengiriman_icon); ?>"></i>
                                <?php echo e($item->jasa_pengiriman_label); ?>

                            </span>
                            <?php if(($item->harga_pengiriman ?? 0) > 0): ?>
                                <span class="kirim-ongkir">
                                    <i class="ri-price-tag-3-line" style="font-size:10px;"></i>
                                    Ongkir: Rp <?php echo e(number_format($item->harga_pengiriman, 0, ',', '.')); ?>
                            ...
                            <?php endif; ?>
                            <?php if(($item->jasa_instalasi ?? 0) > 0): ?>
                                <span class="kirim-instalasi">
                                    <i class="ri-tools-line"></i>
                                    Instalasi: Rp <?php echo e(number_format($item->jasa_instalasi, 0, ',', '.')); ?>
                    ...
                    <td class="right total-value" style="font-size:13px;white-space:nowrap;">
                        ...
                        <?php if(($item->total_terbayar ?? 0) > 0 && !$item->isLunas()): ?>
                        <div style="font-size:11px;color:var(--text-muted);font-weight:400;margin-top:2px;">
                            Terbayar: Rp <?php echo e(number_format($item->total_terbayar, 0, ',', '.')); ?>
                    ...
                    <td class="center">
                        <?php if($item->foto_bukti): ?>
                            <img src="<?php echo e(Storage::url($item->foto_bukti)); ?>"
                                 alt="Bukti"
                                 class="foto-thumb"
                                 onclick="openLightbox('<?php echo e(Storage::url($item->foto_bukti)); ?>', 'Bukti — <?php echo e(addslashes($item->nama_pelanggan)); ?>')"
                    ...
                    <td class="center">
                        <div class="action-wrap">
                            ...
                            <div class="action-dropdown">
                                ...
                                <?php endif; ?>
                                <a href="<?php echo e(route('penjualan.invoice', $item->id)); ?>"
                                ...
                                <button type="button"
                                        ...
                                        onclick="openBuyBackModal(<?php echo e($item->id); ?>, '<?php echo e(addslashes($item->nama_pelanggan)); ?>');closeAllDropdowns();">
                                ...
                                <button type="button"
                                        ...
                                        onclick="openDeleteModal(<?php echo e($item->id); ?>, '<?php echo e(addslashes($item->nama_pelanggan)); ?>');closeAllDropdowns();">
            ...
            <tfoot>
                <tr class="tfoot-total">
                    ...
                    <td class="right total-value" style="font-size:14px;white-space:nowrap;">
                        Rp <?php echo e(number_format($penjualans->sum('total_tagihan'), 0, ',', '.')); ?>
    ...
    <div class="table-footer">
        <div class="pagination-meta">
            Menampilkan
            <strong><?php echo e($penjualans->firstItem()); ?> – <?php echo e($penjualans->lastItem()); ?></strong>
            dari <strong><?php echo e($penjualans->total()); ?></strong> transaksi
        </div>
        ...
        <nav class="pagination-nav" aria-label="Pagination">
            ...
            <?php else: ?>
                <a class="page-btn" href="<?php echo e($penjualans->previousPageUrl()); ?>">
            ...
            <?php $__currentLoopData = $rendered; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $pageItem): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                ...
                <?php else: ?>
                    <a class="page-btn" href="<?php echo e($penjualans->url($pageItem)); ?>"><?php echo e($pageItem); ?></a>
                <?php endif; ?>
            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>

            <?php if($penjualans->hasMorePages()): ?>
                <a class="page-btn" href="<?php echo e($penjualans->nextPageUrl()); ?>">
...
<div class="modal-overlay" id="modalBuyBack">
    <div class="modal modal-xxl">
        ...
        <form id="formBuyBack" method="POST" action="<?php echo e(route('pembelian.buyback.store')); ?>">
            <?php echo csrf_field(); ?>
            ...
            <div class="modal-footer">
                ...
                <button type="submit" id="btnSubmitBuyBack"
...
<script>
const penjualanData = <?php echo json_encode($penjualanData, 15, 512) ?>;
...
function openBuyBackModal(id, nama) {
    ...
    document.getElementById('buyBackPenjualanId').value         = id;
    ...
    document.getElementById('buyBackTableBody').innerHTML = rows;
    calcBuyBackTotal();
...
function calcBuyBackTotal() {
    ...
    document.querySelectorAll('#buyBackTableBody tr').forEach((row, i) => {
        ...
        const qtyInput   = document.getElementById('bb_qty_' + i);
        const hargaInput = document.getElementById('bb_harga_' + i);
        const subCell    = document.getElementById('bb_subtotal_' + i);
...
<?php $__env->stopPush(); ?>
<?php echo $__env->make('admin.layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\laragon\www\paralkesplus\resources\views/admin/penjualan/index.blade.php ENDPATH**/ ?>
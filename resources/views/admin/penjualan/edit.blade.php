...
<style>
    ...
    .current-foto img { width:44px; height:44px; border-radius:6px; object-fit:cover; border:1px solid var(--border); }
    ...
    <div class="form-card-body">
        <div class="form-grid">
            <div class="form-group">
                ...
                <input type="text" name="nama_pelanggan" class="form-control {{ $errors->has('nama_pelanggan') ? 'is-invalid' : '' }}"
            ...
            <div class="form-group">
                ...
                <input type="number" name="diskon_global" class="form-control" id="diskonGlobal"
                       value="{{ old('diskon_global', $penjualan->diskon_global ?? 0) }}" min="0">
            </div>
            ...
            <div class="form-group">
                ...
                <div class="current-foto">
                    <img src="{{ Storage::url($penjualan->foto_bukti) }}" alt="Bukti saat ini">
        ...
        <div class="items-table-wrap">
            <table class="items-table">
                ...
                <tbody id="itemsBody">
                    ...
                    <tr class="item-row"
                        data-index="{{ $i }}"
                        data-inv-id="{{ $detail->inventory_id ?? '' }}"
                        data-harga-baru="{{ $detail->inventory?->harga_jual_baru ?? 0 }}"
                        data-harga-bekas="{{ $detail->inventory?->harga_jual_bekas ?? 0 }}"
                        data-stok-baru="{{ $detail->inventory?->stok_baru ?? 0 }}"
                        data-stok-bekas="{{ $detail->inventory?->stok_bekas ?? 0 }}">
                        <td>
                            <div class="inv-wrap">
                                <input type="hidden" name="items[{{ $i }}][detail_id]"    value="{{ $detail->id }}">
                                <input type="hidden" name="items[{{ $i }}][inventory_id]" class="inv-id-input"
                                       value="{{ old('items.'.$i.'.inventory_id', $detail->inventory_id) }}">
                                <input type="text" name="items[{{ $i }}][nama_barang]"
                                       class="item-input inv-search"
                                       value="{{ old('items.'.$i.'.nama_barang', $detail->nama_barang) }}"
                                       autocomplete="off" required>
                                <div class="inv-dropdown"></div>
                            </div>
                            {{-- Badge stok realtime untuk baris yang sudah punya inventory --}}
                            @if($detail->inventory_id && $detail->inventory)
                            @php
                                $stokEdit = $detail->kondisi === 'bekas'
                                    ? ($detail->inventory->stok_bekas ?? 0)
                                    : ($detail->inventory->stok_baru ?? 0);
                                $clsEdit  = $stokEdit <= 0 ? 'stok-zero' : ($stokEdit <= 3 ? 'stok-low' : 'stok-ok');
                            @endphp
                            <div class="stok-realtime stok-badge {{ $clsEdit }}">
                                {{ $stokEdit <= 0 ? '⚠ Stok habis' : "✓ Tersedia: {$stokEdit} ({$detail->kondisi})" }}
                            </div>
                            @endif
                        </td>
                        <td>
                            <select name="items[{{ $i }}][kondisi]" class="item-select kondisi-select">
                                <option value="baru"  {{ old('items.'.$i.'.kondisi', $detail->kondisi) == 'baru'  ? 'selected' : '' }}>Baru</option>
                                <option value="bekas" {{ old('items.'.$i.'.kondisi', $detail->kondisi) == 'bekas' ? 'selected' : '' }}>Bekas</option>
                            </select>
                        </td>
                        <td>
                            <input type="number" name="items[{{ $i }}][qty]"
                                   class="item-input qty-input"
                                   value="{{ old('items.'.$i.'.qty', $detail->qty) }}"
                                   min="1" required>
                        </td>
                        <td>
                            <input type="text" name="items[{{ $i }}][satuan]"
                                   class="item-input satuan-input"
                                   value="{{ old('items.'.$i.'.satuan', $detail->satuan) }}">
                        </td>
                        <td>
                            <input type="number" name="items[{{ $i }}][harga_satuan]"
                                   class="item-input harga-input"
                                   value="{{ old('items.'.$i.'.harga_satuan', $detail->harga_satuan) }}"
                                   min="0" required>
                        </td>
                        <td>
                            <input type="number" name="items[{{ $i }}][diskon]"
                                   class="item-input diskon-input"
                                   value="{{ old('items.'.$i.'.diskon', $detail->diskon ?? 0) }}"
                                   min="0" max="100">
                        </td>
...
@push('scripts')
<script>
let itemIndex    = {{ $penjualan->details->count() }};
...
function updateStokBadge(row, kondisi) {
    ...
    const lbl  = stok <= 0 ? '⚠ Stok habis' : `✓ Tersedia: ${stok} (${kondisi})`;
        ...
        timer = setTimeout(() => {
            ...
            .catch(err => {
                ...
                dropdown.innerHTML = `<div class="inv-opt" style="color:#EF4444;cursor:default;">
                    <i class="ri-error-warning-line"></i> Gagal memuat (${err.message})
                </div>`;
                dropdown.classList.add('show');
            });
        }, 280);
